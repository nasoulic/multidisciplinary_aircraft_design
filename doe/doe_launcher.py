...
class DoEDialog(QDialog):
    def __init__(self):
        ...
        self.navigate2path(['results', "DoE", "resultsRun_{0}".format(self.run)])
        self.output_dir = os.getcwd()
        os.chdir(self.home_dir)

        self.setWindowTitle("Design of Experiment")
        ...
        self.create_doe_group(grid_layout)
        self.create_buttons(grid_layout)

    def create_doe_group(self, grid_layout):

        doe_group = QGroupBox("Design of Experiment Setup")
        doe_group_layout = QGridLayout()

        doe_group_layout.addWidget(QLabel("Select DoE sampling method"), 0, 0)

        self.method_selection = QComboBox()
        self.method_selection.addItems(["Uniform Latin Hypercube", "More..."])
        self.method_selection.setCurrentIndex(0)
        doe_group_layout.addWidget(self.method_selection, 0, 1)

        doe_group_layout.addWidget(QLabel("Number of samples"), 1, 0)
        self.doe_nsamples = QLineEdit("100")
        doe_group_layout.addWidget(self.doe_nsamples, 1, 1)

        doe_group.setLayout(doe_group_layout)
        grid_layout.addWidget(doe_group, 2, 1)
    ...
    def create_eps_inputs_group(self, grid_layout):
        ...
        self.eps_inputs_cb = {
            "Battery specific energy [kWh/kg]"  : QCheckBox(),
            "Battery specific power [kW/kg]"    : QCheckBox(),
            "Specific power (electronics) [kW/kg]": QCheckBox(),
            "Specific power (motors) [kW/kg]": QCheckBox(),
            "Number of electric motors [-]" : QCheckBox(),
            "Electric motor revolutions per min [rpm]": QCheckBox(),
            "Battery to shaft efficiency [%]": QCheckBox(),
            "System voltage [V]": QCheckBox(),
            "Depth of discharge [%]": QCheckBox(),
            "Battery EOL State of Charge [%]": QCheckBox(),
            # Add more inputs as needed
        }
        ...
        eps_inputs_layout.addWidget(QLabel("Include in DoE"), 0, 4)
        ...
        for key, input_widget in self.eps_inputs.items():
            ...
            self.eps_inputs_cb[key].setEnabled(False)
            ...
            eps_inputs_layout.addWidget(self.eps_inputs_cb[key], row, 4)
            row += 1
    ...
    def create_powertrain_group(self, grid_layout):
        ...
        self.powertrain_specs_defaults_cb = {
            "Fuel to Total Energy ratio" : QCheckBox(),
            "Electric to Total Power ratio" : QCheckBox(),
            "Gas Generator power output [kW]" : QCheckBox(),
            "Gas Turbine SFC reduction compared to EIS 2014 [%]" : QCheckBox(),
            "Thermal Management System Mass [kg]" : QCheckBox(),
        }
        ...
        self.powertrain_layout.addWidget(QLabel("Include in DoE"), 0, 4)
        ...
        for key, item in self.powertrain_specs_defaults.items():
            ...
            self.powertrain_layout.addWidget(self.powertrain_specs_defaults_cb[key], i, 4)
            i += 1
        ...
        self.powertrain_specs_defaults_cb["Fuel to Total Energy ratio"].setDisabled(True)
        ...
        self.powertrain_specs_defaults_cb["Electric to Total Power ratio"].setDisabled(True)
        ...
        self.powertrain_specs_defaults_cb["Thermal Management System Mass [kg]"].setDisabled(True)
        ...
        self.powertrain_specs_defaults_cb["Gas Generator power output [kW]"].setDisabled(True)
        ...
        self.powertrain_specs_defaults_cb["Gas Turbine SFC reduction compared to EIS 2014 [%]"].setDisabled(False)
        ...
        self.powertrain_layout.addWidget(self.launch_gg, 3, 5)
        ...
        self.powertrain_layout.addWidget(self.launch_gt, 4, 5)
        ...
        self.powertrain_layout.addWidget(self.launch_prop, 4, 6)
        ...
        self.powertrain_layout.addWidget(self.launch_tms, 5, 5)
    ...
    def create_buttons(self, grid_layout):
        ...
        self.calculate_button.clicked.connect(self.perform_DoE)
    ...
    def launch_doc_inputs(self):
        ...
        if not self.doc_window:
            self.doc_window = AircraftCostCalculator()
    ...
    def update_input_fields(self):
        ...
        if current_index == 0:
            ...
            self.powertrain_specs_defaults_cb["Fuel to Total Energy ratio"].setDisabled(False)
            ...
            self.powertrain_specs_defaults_cb["Electric to Total Power ratio"].setDisabled(False)
            ...
            self.powertrain_specs_defaults_cb["Thermal Management System Mass [kg]"].setDisabled(False)
            self.powertrain_specs_defaults["Gas Turbine SFC reduction compared to EIS 2014 [%]"].setDisabled(False)
            ...
            for key, item in self.eps_inputs.items():
                ...
                if key != "Number of electric motors [-]" and key != "Electric motor revolutions per min [rpm]":
                    ...
                    self.eps_inputs_cb[key].setEnabled(True)
            ...
            self.powertrain_specs_defaults_cb["Gas Turbine SFC reduction compared to EIS 2014 [%]"].setDisabled(True)
            self.powertrain_specs_defaults_ub["Gas Turbine SFC reduction compared to EIS 2014 [%]"].setText("0")
            ...
            self.powertrain_specs_defaults_cb["Gas Generator power output [kW]"].setEnabled(False)
            ...
            self.powertrain_specs_defaults_lb["Thermal Management System Mass [kg]"].setText("50")
            self.powertrain_specs_defaults_ub["Thermal Management System Mass [kg]"].setDisabled(False)
            self.powertrain_specs_defaults_c["Thermal Management System Mass [kg]"].setDisabled(False)
            self.powertrain_specs_defaults_ub["Thermal Management System Mass [kg]"].setText("150")
            self.powertrain_specs_defaults["Gas Generator power output [kW]"].setEnabled(True)
            self.powertrain_specs_defaults["Gas Generator power output [kW]"].setText("1600")
            self.powertrain_specs_defaults_lb["Gas Generator power output [kW]"].setEnabled(False)
    ...
    def view_aircraft(self):
        self.navigate2path(['results', "DoE", "resultsRun_{0}".format(self.run - 1)])
    ...
    def perform_DoE(self):

        if not int(self.method_selection.currentIndex()) ==  0:
            message = "More DoE sampling methods will be added soon. Switching back to Uniform Latin Hypercube."
            QMessageBox.information(self, "Info", message, QMessageBox.Ok)

        # Create DOE matrix
        include_in_doe_eps = []
        for key, item in self.eps_inputs_cb.items():
            if item.isChecked():
                include_in_doe_eps.append(key)

        include_in_doe_powertrain = []
        for key, item in self.powertrain_specs_defaults_cb.items():
            if item.isChecked():
                include_in_doe_powertrain.append(key)

        factor_eps = lhs(len(include_in_doe_eps), samples = int(self.doe_nsamples.text()))
        factor_powertrain = lhs(len(include_in_doe_powertrain), samples = int(self.doe_nsamples.text()))

        eps_doe_inputs = {}
        for key, item in self.eps_inputs.items():
            eps_doe_inputs[key] = np.array([float(item.text()) for i in range(int(self.doe_nsamples.text()))])

        for i, key in enumerate(include_in_doe_eps):
            eps_doe_inputs[key] = float(self.eps_inputs_lb[key].text()) + factor_eps[:, i]*(float(self.eps_inputs_ub[key].text()) - float(self.eps_inputs_lb[key].text()))

        powertrain_doe_inputs = {}
        for key, item in self.powertrain_specs_defaults.items():
            powertrain_doe_inputs[key] = np.array([float(item.text()) for i in range(int(self.doe_nsamples.text()))])

        for i, key in enumerate(include_in_doe_powertrain):
            powertrain_doe_inputs[key] = float(self.powertrain_specs_defaults_lb[key].text()) + factor_powertrain[:, i]*(float(self.powertrain_specs_defaults_ub[key].text()) - float(self.powertrain_specs_defaults_lb[key].text()))     

        tmp = {}
        for key, value in eps_doe_inputs.items():
            tmp[key] = value.tolist()
        for key, value in powertrain_doe_inputs.items():
            tmp[key] = value.tolist()
        
        for it in range(int(self.doe_nsamples.text())):
        ...
        message = "Calculation completed. Results can be found in the following path:\n {0}resultsRun_{1} - {2}".format("./results/DoE/", self.run - int(self.doe_nsamples.text()) , self.run - 1)
        QMessageBox.information(self, "Calculation Completed", message, QMessageBox.Ok)

        with open("./results/DoE/design_matrix.json", "w+") as myfile:
            json.dump(tmp, myfile, indent = 6)
        myfile.close()
...
class Propulsion(object):
    def __init__(self, powertrain_specs, eps, propeller, gt_from_GUI, powertrain_specs_lb = None, powertrain_specs_ub = None, include_in_doe = None):
        ...
        self.include_in_doe = include_in_doe
        self.eps = eps
        self.propeller = propeller
        self.gt_from_GUI = gt_from_GUI

class EPS(object):
    def __init__(self, eps_inputs, eps_inputs_lb = None, eps_inputs_ub = None, include_in_doe = None):
