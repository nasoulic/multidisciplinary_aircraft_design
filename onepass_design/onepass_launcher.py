...
class OnePassDialog(QDialog):
    def __init__(self):
        ...
        self.navigate2path(['results', "onepass", "resultsRun_{0}".format(self.run)])
        self.output_dir = os.getcwd()
        os.chdir(self.home_dir)
        

        self.setWindowTitle("One Pass Design")
    ...
    def create_buttons(self, grid_layout):
        ...
        self.calculate_button.clicked.connect(self.perform_onepass)
    ...
    def view_aircraft(self):
        self.navigate2path(['results', "onepass", "resultsRun_{0}".format(self.run - 1)])
        out_path = os.getcwd()
        build_aircraft("vsp_aircraft_input_file", self.ac_configuration)
        vsp.ClearVSPModel()
        os.chdir(self.home_dir)
        launch_openvsp(os.path.join(out_path, "Assembly.vsp3"))
    ...
    def perform_onepass(self):
        ...
        propulsion = Propulsion(self.powertrain_specs_defaults, eps, propeller, self.nested_GT_window)
